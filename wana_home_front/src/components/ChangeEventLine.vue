<template>
    <a>
        ...
        <a v-if="change.event_type==='change_owner'">
            持有者从  <b-badge>{{ change.param1 }}</b-badge>变更为 <b-badge>{{ change.param2 }}</b-badge>
        </a>
        <a v-else-if="change.event_type==='start_selling'">
            以 <b-badge>{{ change.param2 }}Gil</b-badge>开始售出（前持有人 <b-badge>{{ change.param1 }}</b-badge> ）
        </a>
        <a v-else-if="change.event_type==='price_reduce'">
            从 <b-badge>{{ change.param1 }}Gil</b-badge> 降价到 <b-badge>{{ change.param2 }}Gil</b-badge>
        </a>
        <a v-else-if="change.event_type==='sold'">
            被 <b-badge>{{ change.param1 }}</b-badge> 购入
        </a>
